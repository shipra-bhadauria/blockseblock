"""
Feature 12: Ship It — starter

This file is the complete Feature 11 application. No new server logic is
needed in main.py for Feature 12. The new work is in:

  starter/Dockerfile          — fill in 3 TODOs
  starter/docker-compose.yml  — fill in 3 TODOs

Once your Dockerfile and docker-compose.yml are complete, run:

    cd week-4-launch/feature-12-ship-it/
    docker compose up --build

Then open http://localhost:8000 — same app, now running in a container.

All Feature 1–11 endpoints work exactly as before.
"""
import base64
import json
import logging
...
_EVAL_CASES_PATH = Path(__file__).parent.parent.parent / "tests" / "eval_cases_example.json"
...
OPENAPI_TAGS = [
    ...
    {"name": "F3 · AI Memory", "description": "**Week 1 · Feature 3** — Session management and sliding-window history."},
    {"name": "F4 · Feed the Brain", "description": "**Week 2 · Feature 4** — Document ingestion (PDF, DOCX, TXT)."},
    {"name": "F5 · Find the Answer", "description": "**Week 2 · Feature 5** — Semantic search over document chunks."},
    {"name": "F6 · Smart Router", "description": "**Week 2 · Feature 6** — Intelligent LLM/RAG/hybrid routing."},
    {"name": "F7 · First Agent", "description": "**Week 3 · Feature 7** — ReAct tool-calling agent."},
    {"name": "F8 · Multi-Step Agent", "description": "**Week 3 · Feature 8** — Plan-and-Execute background agent."},
    {"name": "F9 · MCP Integration", "description": "**Week 3 · Feature 9** — Model Context Protocol tool servers."},
    {"name": "F10 · Multimodal AI", "description": "**Week 4 · Feature 10** — Voice (STT/TTS) and Vision (VLM)."},
    {"name": "F11 · Production Design", "description": "**Week 4 · Feature 11** — Observability and eval harness."},
    {"name": "Infrastructure", "description": "Health check, provider info, live metrics."},
]
...
async def lifespan(app: FastAPI):
    ...
    _log.info("Feature 12 starter server started", extra={"event": "startup"})
    yield
...
app = FastAPI(
    ...
    description=(
        ...
        "Feature 12 starter: fill in Dockerfile and docker-compose.yml TODOs, then run:\n"
        "  cd week-4-launch/feature-12-ship-it/ && docker compose up --build"
    ),
    version="12.0.0",
    lifespan=lifespan,
...
async def vision_analyze(image: UploadFile = File(...), prompt: str = Form("Describe this image in detail."), detail: str = Form("auto")) -> VisionAnalyzeResponse:
...
async def vision_chat(image: UploadFile = File(...), prompt: str = Form("Describe this image in detail."), detail: str = Form("auto"), session_id: str = Form(""), tenant_id: str = Depends(get_tenant_id)) -> VisionAnalyzeResponse:
    image_bytes = await image.read()
    result = await analyze_image(image_bytes, prompt, detail)
    answer = result.content or ""
    if session_id:
...
async def multimodal_chat(session_id: str = Form(...), message: str = Form(""), audio: UploadFile | None = File(None), image: UploadFile | None = File(None), prompt: str = Form(""), tenant_id: str = Depends(get_tenant_id)) -> MultimodalChatResponse:
    ...
    if modality == "voice":
        ...
        try:
            ...
            pass
        return MultimodalChatResponse(modality="voice", answer=smart_resp.answer, source=smart_resp.source, chunks_used=list(smart_resp.chunks_used), confidence=smart_resp.confidence, retrieval_method=smart_resp.retrieval_method, transcript=transcript, audio_base64=audio_base64, audio_mime="audio/mpeg")
    if modality == "vision":
    ...
    return MultimodalChatResponse(modality="text", answer=smart_resp.answer, source=smart_resp.source, chunks_used=list(smart_resp.chunks_used), confidence=smart_resp.confidence, retrieval_method=smart_resp.retrieval_method)
...
async def eval_run() -> EvalReport:
    if not _EVAL_CASES_PATH.exists():
        raise HTTPException(status_code=404, detail=f"Eval cases file not found at {_EVAL_CASES_PATH}.")
...
async def health() -> dict:
    from shared.config import settings
    return {"status": "ok", "version": "12.0.0", "provider": settings.llm_provider, "metrics": metrics.get_metrics()}
